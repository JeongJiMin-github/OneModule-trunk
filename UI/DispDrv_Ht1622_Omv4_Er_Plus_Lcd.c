...
* @brief   Display Driver Source File\n
           for OMV4 ER PLUS M LCD
* @file    DispDrv_Ht1622_Omv4_Er_Plus_Lcd.c
* @version 1.0
* @date    2016/12/06
...
#ifdef DISP_DRV_HT1622_M0516LDN_ER_PRO
/*
********************************************************************************
...
#define DISP_DRV_TOTAL_PRICE_ADDR 16
...
#define DISP_DRV_ADDR_ZERO			{14, 0x02}//6j weight
/** @brief NET indicator */
#define DISP_DRV_ADDR_NET				{14, 0x01}//5j weight
/** @brief Low Battery indicator*/
#define DISP_DRV_ADDR_LOW_BATT		{14, 0x20}//6j unit price
/** @brief Euro per kilogram  indicator */
#define DISP_DRV_ADDR_EURO_KG			{12, 0x80}//4j unit price
/** @brief Euro indicator */
#define DISP_DRV_ADDR_EURO			{22, 0x40}//6j total price {14, 0x02}
/** @brief Backlight indicator*/
#define DISP_DRV_ADDR_BACKLIGHT		{14, 0x10}//5j unit price
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_KG				{12, 0x08}//4j weight
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_LB				{12, 0x04}//3j weight
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_TARESAVE		{12, 0x01}//1j weight
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_UNITPRICE_KG	{12, 0x20}//2j unit price
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_UNITPRICE_LB	{12, 0x10}//1j unit price
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_UNITPRICE_100G	{12, 0x40}//3j unit price
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_BOTHSAVE		{24, 0x20}//4j total price
/** @brief Prepack indicator*/
#define DISP_DRV_ADDR_PREPACK			{12, 0x80}//4j unit price same EURO, alternate use
...
#define DISP_DRV_CHAR_MASK	0xF7
...
/** @brief s0~s15 중 디지트 시작 주소*/
INT8U code dispDrvCharAddrTableForTotal[8] = { 0, 1, 2, 3, 4, 5, 10, 11};
...
const INDICATOR_ADDR code dispDrvDotAddrTableForTotal[8] =
{
	{0, 0x08}, {1, 0x08}, {2, 0x08}, {3, 0x08}, {4, 0x08}, {5, 0x08}, {10, 0x08}, {11, 0x00}
};
const INDICATOR_ADDR code dispDrvCommaAddrTableForTotal[8] =
{
	{7, 0x08}, {7, 0x80}, {7, 0x40}, {9, 0x04}, {9, 0x20}, {9, 0x10}, {9, 0x02}, {9, 0x00}
};
...
void DispDrvSetChar(INT8U winNum, INT8U pos, char cdata)
{
	...
	{		
		addr += dispDrvCharAddrTableForTotal[pos];		
		byte = dispDrvBuf[dispEditingPage][addr];
		byte = byte & (~DISP_DRV_CHAR_MASK);
		dispDrvBuf[dispEditingPage][addr] = (FontTable[cdata - 0x20] & DISP_DRV_CHAR_MASK) | byte;
	}
}
...
void DispDrvSetStr(INT8U winNum, char *str)	 
{
	...
	for (i = 0; i < dataSize; i++) 
	{
		...
		{
			addr = dispDrvCharAddrTableForTotal[i] + DISP_DRV_TOTAL_PRICE_ADDR; 	
			byte = dispDrvBuf[dispEditingPage][addr];
			byte = byte & (~DISP_DRV_CHAR_MASK);
			dispDrvBuf[dispEditingPage][addr] = (FontTable[dispChar - 0x20] & DISP_DRV_CHAR_MASK) | byte;
		}
	}
}
...
void dispDrvSetDot(INT8U winNum, INT8U decPos, INT8U on)
{
	...
	if (on)
	{
		...
		{
			addr += dispDrvDotAddrTableForTotal[pos].addr;
			mask = dispDrvDotAddrTableForTotal[pos].mask;
		}
		else
		{
			addr += dispDrvDotAddrTable[pos].addr;
...
void ht1622TranCommand(INT8U command)      
{
	...
	for (k = 0; k < 8; k++)		
	{
		...
		HT1622_WR_HIGH;   //Data are latched into the HT1622
		command <<= 1;
		delay2Clock();
	}
}
...
//	IEN0 = 0x0a;		// Disable all, Enable T1,0 Timer overflow
	...
	HT1622_CS_HIGH;
    ht1622TranCommand(0x0f);

    ht1622TranCommand(0x03); //Write from memory address 0
	
//	IEN0 = 0x8a;        // Enable all, Enable T1,0 Timer overflow
}
#endif//#ifdef DISP_DRV_HT1622_M0516LDN_ER_PRO
...
#elif  defined (TURKEY_BLOCK_FUNCTION_KEY) 	//Turkey Exclusive Key Setting (Only use the Zero, Tare, On/off key) 
...
#ifdef TURKEY_BLOCK_FUNCTION_KEY
	blockkey = ParamReadByte(PARAM_DISP_AREA_NUM, DISP_PARAM_OFFSET_BLINK_COUNT);
	blockkey = blockkey & 0x01;
	if( ScaleMode == SCALE_MODE_CAL) blockkey = 0;
	funcKey = KeyDrvDefaultFuncTable[blockkey][rawKey]; 
#else
...
#endif //#ifdef KEY_DRV_OMV4_ED_LCD

...
#define DISP_DRV_ADDR_PCS				{0, 0x10}
/** @brief Zero indicator*/
#define DISP_DRV_ADDR_ZERO		     	{0, 0x80}
/** @brief High indicator, for comparator*/
//#define DISP_DRV_ADDR_HOLD			{0, 0x08}
#define DISP_DRV_ADDR_HOLD			{0, 0x40}
/** @brief Low indicator, for comparator*/
//#define DISP_DRV_ADDR_LOW				{0, 0x40}
...
{
	{5, 0x80}, {4, 0x80}, {3, 0x80}, {2, 0x80}, {1, 0x80} 
};
...
void DispDrvProcForInterrupt(void)
{
	...
	dispintdelay ++;
	if(dispintdelay%10 <= pgmBlBrightness + 1)
	{
		LED_INDICATOR   = OFF;
		LED_SEG_DIGIT_1 = OFF;
		LED_SEG_DIGIT_2 = OFF;
		...
		if(digitCount != DISP_DIGIT_INDICATOR)						   //LED 잔상 제거를 위한 부분
		{
			DIGIT_CON=1;
		...
		switch (digitCount)
		{
			case DISP_DIGIT_INDICATOR:
				DIGIT_CON=1;
				LED_SEG_DATA = dispDrvBufForInterrupt[digitCount];		//LED 잔상 제거를 위한 부분
				DIGIT_CON=0;
				LED_SEG_DATA = 0x00; 
				LED_INDICATOR   = ON;				
				break;			

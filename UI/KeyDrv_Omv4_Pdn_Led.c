...
* @brief   Display Driver Source File\n
           for OMV4 PDN LED
* @file    KeyDrv_Omv4_Pdn_Led.c
* @version 1.0
* @date    2020/04/08
* @author  Lee Dong Won
********************************************************************************
* @remark  [Version History]\n
*          v1.0 2020/04/08 created by Lee Dong Won \n
********************************************************************************
*/
...
#define KEY_DRV_PROC_TIME_INTERVAL	1//단위 ms //100ms
...
{
	{
		KEY_ZERO, KEY_TARE,	KEY_UNIT_CHANGE
	},
	{
		KEY_ZERO, KEY_TARE,	KEY_PRINT
	},
	{
		KEY_ZERO, KEY_TARE,	KEY_HOLD
	}
};
/**
...
const KEY_TYPE code KeyDrvFuncTableForPgm[3][KEY_MAX_KEY_NUM_FOR_PGM] = 
{
	...
	{
		KEY_ZERO, KEY_DUMMY, KEY_TARE,	 KEY_PRINT, KEY_PRINT, KEY_TARE, KEY_NOT_EXISTED, KEY_NOT_EXISTED, KEY_NOT_EXISTED
	},
	{
...
{  
	...
	else
	{
		...
		funcKey = KeyDrvDefaultFuncTable[keyDrvType][0][rawKey];
#ifdef USE_LONG_KEY
		if (keyDrvLongKeyFlag == ON)
		{
			if (((funcKey == KEY_UNIT_CHANGE)|| (funcKey == KEY_TARE)||(funcKey == KEY_HOLD)) && (keyDrvPressCount != 0) && !(RingBufCheckBuf(&KeyDrvRingBuf)))
			{
				if (keyDrvPressCount > KeyDrvLongValidCount)
				{
					switch(funcKey)
					{
						case KEY_UNIT_CHANGE:
							funcKey = KEY_PRINT;
							break;
						case KEY_TARE:
...
#endif//#ifdef KEY_DRV_OMV4_PDN_LED

...
#ifdef DISP_DRV_74HC573_ER_MLED
...
#define DISP_DRV_UNIT_PRICE_SIZE 5
...
#define DISP_DRV_ADDR_STABLE			{18, 0x02}
/** @brief NET indicator*/
#define DISP_DRV_ADDR_NET				{18, 0x04}
/** @brief Low Battery indicator*/
#define DISP_DRV_ADDR_BATT_FULL			{18, 0x08}
/** @brief PCS indicator, for counting*/
//#define DISP_DRV_ADDR_PCS				{18, 0x10}
/** @brief Zero indicator*/
#define DISP_DRV_ADDR_ZERO				{18, 0x01}
/** @brief High indicator, for comparator*/
//#define DISP_DRV_ADDR_HI				{18, 0x08}
/** @brief Low indicator, for comparator*/
//#define DISP_DRV_ADDR_LOW				{18, 0x40}
...
static INT8U idata digitViewPort[24];//,digitChanged;
INT8U displaySleep;
/**
INT8U digitCount;
...
const INDICATOR_ADDR code dispDrvIndicatorAddrTable[INDICATOR_MAX_NUM] =
{
	...
	DISP_DRV_ADDR_BATT_FULL, 			///7 Low battery  (Batt. Full???)
	{0, 0}, 							///8 Ounce
	{0, 0},								///9 Pound
	{0, 0},								//10 Kilo gram
	{0, 0}, 							//11 Gram
...
{
	for (digitCount=0; digitCount<24; digitCount++) {
...
********************************************************************************
*/
extern INT8U keyInByte;
INT8U digitID=0;

//extern INT8U keytempByte;

//INT8U keydispByte;
extern INT8U PowerCheck();
...
void DispDrvProc(void)
{
	...
	if (displaySleep) {
		...
		digitViewPort[0]=0x10;	// a
		digitViewPort[3]=0x10;	// b
		digitViewPort[6]=0x10;	// c
		digitViewPort[9]=0x10;	// d
		digitViewPort[12]=0x10;	// e
		digitViewPort[15]=0x10;	// f
		if (dispDrvBuf[dispTransPage][18]&0x08) {
			digitViewPort[11]=0x04;
		}
		//digitViewPort[18]=0x00; // g
		//digitViewPort[21]=0x00; // dot
		return;
	}
	if (digitRun) return;
/////////////
//	if (PowerCheck()) {
//		dispDrvBuf[dispTransPage][0]=FontTable[0x31-0x20];
//	} else {
//		dispDrvBuf[dispTransPage][0]=FontTable[0x30-0x20];
//	}
/*	if (keyInByte) {
		si=KeyOutputPortStatus/16;
		if (si>=10) si+='A'-10;
		else si+='0';
		di=KeyOutputPortStatus&0x0f;
		if (di>=10) di+='A'-10;
		else di+='0';
		dispDrvBuf[dispTransPage][0]=FontTable[si-0x20];
		dispDrvBuf[dispTransPage][1]=FontTable[di-0x20];
	}
		si=keyInByte/16;
		if (si>=10) si+='A'-10;
		else si+='0';
		...
		dispDrvBuf[dispTransPage][2]=FontTable[si-0x20];
		dispDrvBuf[dispTransPage][3]=FontTable[di-0x20];
	}
**/
	di=0;
	digit_calDMask=0x01;
REPT2:
	digit_calSMask     =0x01;
	digitViewPort[di  ]=0;
	digitViewPort[di+1]=0;
	digitViewPort[di+2]=0;
	for (si=0; si<19; si++) {
		dv = dispDrvBuf[dispTransPage][si];
		dv&= digit_calDMask;
		pos= si>>3;
		if (dv) {
			digitViewPort[di+pos]|=digit_calSMask;
		}
		digit_calSMask<<=1;
...
void DispDrvProcForInterrupt(void)
{
	...
	digit1=digitCount+digitCount+digitCount;
	LED_SEG_DATA   = 0;
	LED_SEG1_LATCH = 1;
	LED_SEG2_LATCH = 1;
	LED_SEG3_LATCH = 1;
	...
	LED_SEG_DATA   = digitViewPort[digit1 ];
	LED_SEG1_LATCH = 1;	// 0:Latch Enable
	delay1Clock();
	LED_SEG1_LATCH = 0;
	digit1++;
	LED_SEG_DATA   = digitViewPort[digit1 ];
	LED_SEG2_LATCH = 1;	// 0:Latch Enable
	delay1Clock();
	LED_SEG2_LATCH = 0;
	digit1++;
	LED_SEG_DATA   = digitViewPort[digit1 ];
	LED_SEG3_LATCH = 1;	// 0:Latch Enable
	delay1Clock();
	LED_SEG3_LATCH = 0;
...
//	keyInByte&=0xf0;
//	if (digitCount<4) {
//		if (keyInByte==0) digitID=digitCount+1;
//	}
//	if (digitCount>4) {

...
* @brief   Display Driver Source File\n
           for OMV4 SW LCD
* @file    DispDrv_Ht1621_Omv4_Sw_Lcd.c
* @version 1.0
* @date    2016/12/06
...
#ifdef DISP_DRV_HT1621_M0516LDN
/*
********************************************************************************
...
void DispDrvSetDecPoint(INT8U winNum, INT8U decPos, INT8U decType, INT8U useThousandPt)
{
	...
	if (decPos == 0xFE) //version string for old certi
	{
		dispDrvSetDot(winNum, 1, ON);
		dispDrvSetDot(winNum, 3, ON);
		return;
	}
	if (decPos != 0)
	{
...
void ht1621TranCommand(INT8U command)      
{
    ...
    for (k = 0; k < 8; k++)
	{
		...
		HT1621_WR_HIGH;   //Data are latched into the HT1621
		command <<= 1;
		delay2Clock();
	}  
}
...
#endif//#ifdef DISP_DRV_HT1621_M0516LDN

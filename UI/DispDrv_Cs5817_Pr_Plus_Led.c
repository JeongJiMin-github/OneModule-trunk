...
#ifdef DISP_DRV_CS5817_PR_PLUS_LED
/*
********************************************************************************
...
#define DISP_DRV_UNIT_PRICE_ADDR 5
...
#define DISP_DRV_TOTAL_PRICE_ADDR 10
/** @brief Total price display size*/
#define DISP_DRV_TOTAL_PRICE_SIZE 6

/** @brief Stable indicator*/
#define DISP_DRV_ADDR_STABLE			{4, 0x20}
/** @brief NET indicator*/
#define DISP_DRV_ADDR_NET				{0, 0x20}
#ifdef USE_UNIT_CHANGE_FUNCTION
/** @brief Pound indicator*/
#define DISP_DRV_ADDR_LB				{10, 0x20}
#else
/** @brief Low Battery indicator*/
#define DISP_DRV_ADDR_LOW_BATT			{10, 0x20}
#endif
/** @brief Zero indicator*/
#define DISP_DRV_ADDR_ZERO				{1, 0x20}
/** @brief Charging Battery indicator*/
#define DISP_DRV_ADDR_CHG_BATT			{5, 0x20}
/** @brief AC indicator*/
#define DISP_DRV_ADDR_AC				{9, 0x20}
...
void DispDrvSetChar(INT8U winNum, INT8U pos, char cdata)
{
	...
	addr = dispDrvStruct[winNum].addr + dispDrvCharAddrTable[dataSize-pos-1];
	byte = charChange(FontTable[cdata - 0x20]);     // duckspg
	byte = byte & (~0x20);
	dispDrvBuf[dispEditingPage][addr] = dispDrvBuf[dispEditingPage][addr] & 0x20;
...
void DispDrvSetAllIndicator(INT8U on)
{
	...
	for (winNum = 0; winNum < DISP_DRV_WIN_MAX_NUM; winNum++)
	{	
		addr = dispDrvStruct[winNum].addr + dispDrvCharAddrTable[pos];
		size = dispDrvStruct[winNum].size;
		for (pos = 0; pos < size; pos++)
		{
...
void DispDrvSetStr(INT8U winNum, char *str)
{
	...
	for (i = 0; i < dataSize; i++) 
	{
		...
		addr = dispDrvStruct[winNum].addr + dispDrvCharAddrTable[(dataSize-i)-1];		
...
void DispDrvProc(void)
{
	...
	if(dispDrvTurnOnOff)
	{
		for (i = DISP_OFFSET,j=0; i <= (DISP_OFFSET + (DISP_DRV_BUF_SIZE<<1)) ; i=i+2)  // duckspg pcb 아트웍 라인때문에 DIG4는 연결안했음 
		{
            dispCs5817(i,dispDrvBuf[dispTransPage][j]);
            j++;
		}
	}
	else
...
void ledClear(void)
{
    ...
    for (i = DISP_OFFSET; i < (DISP_OFFSET + 32) ; i=i+2)
    {
        dispCs5817(i,0x00);
    }    
}
...
void dispDrvSetDot(INT8U winNum, INT8U decPos, INT8U on)
{
	...
	addr = dispDrvStruct[winNum].addr + dispDrvDotAddrTable[decPos].addr;
	mask = dispDrvDotAddrTable[decPos].mask;

	for(i=0;i<INDICATOR_MAX_NUM;i++){
		if(addr == dispDrvIndicatorAddrTable[i].addr)return;
	}
	if (on)
	{
		dispDrvBuf[dispEditingPage][addr] = dispDrvBuf[dispEditingPage][addr] | mask;
...
INT8U charChange(INT8U charFont)
{
    ...
    tempChangeByte.bit.c_h = (charFont>>5) & 0x01;
    tempChangeByte.bit.c_f = (charFont>>7) & 0x01;	
    return tempChangeByte.changebyte;
}

#endif//#ifdef DISP_DRV_STM8L151C8

...
* @file    KeyDrv_Omv4_Er_Plus_Lcd.c
* @version 1.0
* @date    2016/12/06
...
/** @brief key input Byte Table */
const INT8U code KeyInTable[8] =
{
...
/*	for (i = 0; i < KEY_DRV_MAX_KEY_NUM; i++) //csh 20120730
	...
	//if (keyModelNum != MODEL_ER_PRO_LCD)
...
//	INT8U keyInByte, keyOutByte;
	...
	volatile INT8U keyInByteTemp;

	keyInDword = KEY_IN_PORT;
        keyInByte = (INT8U)((keyInDword & 0x000000ff));
        
	keyInByteTemp = (INT8U)(KEY_IN_PORT_REMAIN1 << 6) & 0x40;
	keyInByteTemp |= ((INT8U)(KEY_IN_PORT_REMAIN2 << 7) & 0x80);	//add
	keyInByte = ((keyInByte >> 2) & KEY_IN_MASK);
	keyInByte = keyInByte | keyInByteTemp; 
	switch (keyInByte)	//kkh0225 code optimaization
	{
		case 0xFE:
	...
	KEY_OUT1 = (keyDrvIndex & 0x02)>>1;//bug fix
	KEY_OUT2 = (keyDrvIndex & 0x04)>>2;//bug fix
...
#endif//#ifdef KEY_DRV_OMV4_ER_PRO_LCD

...
#define DISP_DRV_TOTAL_PRICE_INDICATOR_ADDR 	25
...
#define DISP_DRV_ADDR_LOW_BATT				{28, 0x10}
/** @brief Euro indicator, triangle 5*/
//#define DISP_DRV_ADDR_EURO				{26, 0x20}
#define DISP_DRV_ADDR_EURO					{28, 0x20}
...
#define DISP_DRV_ADDR_PREPACK				{27, 0x10}
/** @brief Backlight indicator*/
//#define DISP_DRV_ADDR_BACKLIGHT			{26, 0x04}
#define DISP_DRV_ADDR_BACKLIGHT			{28, 0x04}
...
#define DISP_DRV_ADDR_BOTHSAVE				{27, 0x20}//4j total price
...
const INDICATOR_ADDR code dispDrvCommaAddrTableForTotalWin[7] =
{
	{3, 0x40}, {3, 0x80},{2, 0x40}, {2, 0x80}, {1, 0x40}, {1, 0x80}, {0, 0x40}
};
...
const INDICATOR_ADDR code dispDrvBattAddrTable[5] =
{
	...
	{27, 0x04}, {28, 0x02}, {28, 0x01}, {27, 0x02}, {27, 0x01} 
};
...
void dispDrvSetComma(INT8U winNum, INT8U decPos, INT8U on)
{
	...
	if (winNum == WIN_TOTAL_PRICE)
	{
		noUseDigit = DISP_DRV_TOTAL_PRICE_SIZE - dispDrvStruct[winNum].size;
		pos = dispDrvStruct[winNum].size - decPos - 1;
		pos = pos + noUseDigit;
		addr = DISP_DRV_TOTAL_PRICE_INDICATOR_ADDR;
		addr = addr + dispDrvCommaAddrTableForTotalWin[pos].addr;
		mask = dispDrvCommaAddrTableForTotalWin[pos].mask;
	}
	else
	{
		noUseDigit = DISP_DRV_WEIGHT_SIZE - dispDrvStruct[winNum].size;//DISP_DRV_WEIGHT_SIZE == DISP_DRV_UNIT_PRICE_SIZE
		pos = dispDrvStruct[winNum].size - decPos - 1;
		pos = pos + noUseDigit;
		if (winNum == WIN_WEIGHT) 
		{
			addr = DISP_DRV_WEIGHT_INDICATOR_ADDR;
		}
		else
		{
			addr = DISP_DRV_UNIT_PRICE_INDICATOR_ADDR;
		}

...
/* Insert #define here */
/** @brief key 개수 */
#define KEY_DRV_MAX_KEY_NUM	7

/** @brief key ring buffer size */
#define KEY_DRV_BUF_SIZE	7
...
{
//	pwr 0x04 0x08 0x10    0x20    0x40   0x80
	0,	 1,	  2,	   3,        4 ,        5,       6
};
...
{
	{
		//KEY_ON_OFF,  KEY_STAR,   KEY_ZERO,  KEY_TARE,	KEY_HOLD,   KEY_C,  KEY_SUM
		KEY_ON_OFF,  KEY_UNIT_CHANGE,   KEY_ZERO,  KEY_TARE,	KEY_HOLD,   KEY_C,  KEY_SUM
	}
};
...
{
	//KEY_HOLD, KEY_STAR,	KEY_ZERO,  KEY_TARE, KEY_ZERO,  KEY_TARE, KEY_NOT_EXISTED, KEY_NOT_EXISTED, KEY_NOT_EXISTED
	KEY_HOLD, KEY_UNIT_CHANGE,	KEY_ZERO,  KEY_TARE, KEY_ZERO,  KEY_TARE, KEY_NOT_EXISTED, KEY_NOT_EXISTED, KEY_NOT_EXISTED
};
...
{  
    if (POWER_KEY_CHK == 0)
	{
   	...
   	else if (KEY_PORT_STAR == 0)
	{
		keyDrvRawData = 1;
	}
	else if (KEY_PORT_ZERO == 0)
	{
		keyDrvRawData = 2;
	}
	...
	else if (KEY_PORT_HOLD == 0)
	{
		keyDrvRawData = 4;
	}	
	else if (KEY_PORT_SET == 0)
	{
		keyDrvRawData = 5;
	}	
	else if (KEY_PORT_SUM == 0)
	{
		keyDrvRawData = 6;
	}	
	else
	...
	#ifdef COUNTRY_USA
	//else if((keyDrvRawData == 0x01) && (SaleData.saleState == SALE_STATE_NORMAL || SaleData.saleState == SALE_STATE_UNITCHANGE))	//Long Key Unit Change
	else if((keyDrvRawData == 0x01) && (SaleData.saleState == SALE_STATE_NORMAL))	//Long Key Unit Change
	{
		//Key is pressed
		if (keyDrvValidRawData == keyDrvRawData)
		{
			keyDrvPressCount++;
			if (keyDrvPressCount == 0x4650)		//10초간 누르고 있으면
			{
				if (KeyDrvEnableFlag)
				{
...
#endif//#ifdef KEY_DRV_CASTON_LED

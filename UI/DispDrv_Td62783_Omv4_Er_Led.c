...
* @file    DispDrv_Td62783_Omv4_Er_Led.c
* @version 1.0
* @date    2016/12/06
...
#ifdef DISP_DRV_TD62783_M0516LDN_ER_LED
/*
********************************************************************************
...
#define DISP_DRV_ADDR_ZERO			{18, 0x01}//6j weight
/** @brief NET indicator */
#define DISP_DRV_ADDR_NET				{18, 0x02}//5j weight
/** @brief Low Battery indicator*/
#define DISP_DRV_ADDR_LOW_BATT		{18, 0x40}//6j unit price
/** @brief Euro per kilogram  indicator */
#define DISP_DRV_ADDR_EURO_KG			{18, 0x80}//5j unit price
/** @brief Euro indicator */
#define DISP_DRV_ADDR_EURO			{19, 0x10}//4j total price
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_KG				{18, 0x04}//4j weight
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_LB				{18, 0x08}//3j weight
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_OZ				{18, 0x10}//3j weight
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_TARESAVE		{18, 0x20}//1j weight
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_UNITPRICE_KG	{19, 0x04}//2j unit price
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_UNITPRICE_LB	{19, 0x08}//1j unit price
#ifdef USE_UNIT_SPENISH_OZ
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_UNITPRICE_OZ	{19, 0x02}//1j unit price
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_UNITPRICE_100G	{19, 0x01}//3j unit price
#else
/** @brief 100g KEY indicator*/
#define DISP_DRV_ADDR_UNITPRICE_100G	{19, 0x02}//3j unit price
#endif
#define DISP_DRV_ADDR_BOTHSAVE		{19,0x20}//3j total price
/** @brief Prepack indicator*/
#define DISP_DRV_ADDR_PREPACK		{19, 0x10}//4j total price same EURO, alternate use
...
void DispDrvProcForInterrupt(void)
{
	...
	LED_DIGIT_DATA(0x00);
	LED_DIGIT_01TO08_LATCH = 1;	// 0:Latch Enable
	delay1Clock();
	LED_DIGIT_01TO08_LATCH = 0;
	
	LED_DIGIT_DATA(0x00);
	LED_DIGIT_09TO16_LATCH = 1;
	delay1Clock();
	LED_DIGIT_09TO16_LATCH = 0;
	
	LED_DIGIT_DATA(0x00);
	LED_DIGIT_17TO20_LATCH = 1;
	delay1Clock();
	LED_DIGIT_17TO20_LATCH = 0;
	
	LED_SEG_DATA(dispDrvBufForInterrupt[digitCount]);
	LED_SEG_LATCH = 1;	// 0:Latch Enable
	delay3Clock();
	LED_SEG_LATCH = 0;	// 0:Latch Enable
	
	LED_DIGIT_DATA(digitOut);
	LED_DIGIT_01TO08_LATCH = 1;	// 0:Latch Enable
	delay1Clock();
	LED_DIGIT_01TO08_LATCH = 0;
	
	LED_DIGIT_DATA((digitOut>>8)&0xff);
	LED_DIGIT_09TO16_LATCH = 1;
	delay1Clock();
	LED_DIGIT_09TO16_LATCH = 0;
	
	LED_DIGIT_DATA((digitOut>>16)&0xff);
	LED_DIGIT_17TO20_LATCH = 1;
	delay1Clock();
	LED_DIGIT_17TO20_LATCH = 0;

	// keyDrvIndex update //
	KeyOutputPortStatus = digitCount;
	////////////////////////
...
#endif//#ifdef DISP_DRV_TD62783_M0516LDN_ER_LED

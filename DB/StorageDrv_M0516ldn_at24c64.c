...
* @brief   Storage Drvier Source File\n
           Chip : AT24C64D
* @file    StorageDrv_M0516ldn_at24c64.c
* @version 1.0
* @date    2017/04/20
...
#define AT24C64D_DEVICE_WRITE_ADDR 	    0xA2
/** @brief EEPROM_DEVICE READ ADDRESS ( A2 : 0, A1 : 0, A0 : 1) */
#define AT24C64D_DEVICE_READ_ADDR       0xA3
...
static void at24c64dWriteByte(INT8U pageAddr, INT8U lowAddr, INT8U src);
static void at24c64dReadByte(INT8U pageAddr, INT8U lowAddr, INT8U *dst);
...
void StorageDrvWriteData(INT16U addr, INT8U *src, INT16U size)
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        at24c64dWriteByte(highAddr, lowAddr, src[i]);
...
void StorageDrvReadData(INT16U addr, INT8U *dst, INT16U size)
{
    ...
    for (i = 0; i < size; i++)
    {
      ...
      at24c64dReadByte(highAddr, lowAddr, dst+i);
...
/**
********************************************************************************
* @brief    AT24C64D에 Write 신호를 보내는 함수\n
* @param  pageAddr : 주소상위 값, lowAddr : 주소 하위 값, src : Write하려는 값
* @return   none
* @remark
********************************************************************************
*/
void at24c64dWriteByte(INT8U pageAddr, INT8U lowAddr, INT8U src)
{
Rewrite1:
	ackOk = 1;
	tsiSendStartBit();
	tsiWriteByte(AT24C64D_DEVICE_WRITE_ADDR);
	tsiAckPollFromSlave();
	if(!ackOk)
	{
		goto Rewrite1;
...
/**
********************************************************************************
* @brief    AT24C64D에 Read 신호를 보내는 함수\n
* @param  pageAddr : 주소상위 값, lowAddr : 주소 하위 값, src : Read하려는 위치
* @return   none
* @remark
********************************************************************************
*/
void at24c64dReadByte(INT8U pageAddr, INT8U lowAddr, INT8U *dst)
{
Rewrite2:
	ackOk = 1;	
	tsiSendStartBit();   	
	...
	tsiAckPollFromSlave();
	tsiSendStartBit();
	tsiWriteByte(AT24C64D_DEVICE_READ_ADDR);
	tsiAckPollFromSlave();
	*dst = tsiReadByte();
	tsiSendNak();
	tsiSendStopBit();
...
#endif//#ifdef STORAGE_DRV_AT24C16D

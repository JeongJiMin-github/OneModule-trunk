...
* @brief   Hardware Configuraiton Header File\n
           for TM LCD
* @file    HardwareConfig_At89c51ed2_Omv3_Tm_Lcd.h
* @version 1.0
* @date    2012/10/10
...
#ifndef _HARDWARE_CONFIG_EFM32G840F128_OMV3_TM_LCD_H
#define _HARDWARE_CONFIG_EFM32G840F128_OMV3_TM_LCD_H
...
/** @brief Device Option : use EFM32G Display Driver IC for TM LCD */
#define DISP_DRV_EFM32G840_OMV3_TM_LCD
/** @brief USE EFM32G Buzzer Driver */
#define BUZZER_DRV_EFM32G
/** @brief USE EFM32G Relay Driver */
#define RELAY_DRV_STD
/** @brief KEY_DRV_TM_LCD key driver*/
#define KEY_DRV_OMV3_TM_LCD
...
/** @brief Device Option : use FW(LCD) Touch Key Driver */
//#define KEY_DRV_NSW_TOUCH_LCD      // duckspg  터치를 하기 위한 DEFINE 이것은 나중에 없엘수 있다. KEY_DRV_FW_LCD과 KEY_DRV_FW_TOUCH_LCD는 동시에 define 될수 없다 
                            // 터치를 하기 위해서 메인 timer를 letimer로 변경 Timer0를 touch로 쓰기 위해서 
                            // letimer는 low fq로 돌기 때문에 정확한 1ms를 맞출수 없다 따라서 delay를 사용하는 timer1으로는 대체하기 힘들다 
...
#define EFM32G840F128_MAINCLK       7       // cpu main clk 예 1Mhz설정 = 1, 7Mhz설정 = 7
#define EFM32G840F128_PERCLK        0       // 2^x X승의 값 divide 예 3이면 8로 나눔 예 : cpu를 7Mhz로 설정한후 3을 설정하면 Peripheral clk은 0.875Mhz로 설정됨
...
#include "../GeckoConfig/efm32lib/inc/efm32_gpio.h"
#include "efm32_adc.h"
#include "efm32_cmu.h"
...
#define KEY_PORT_UNIT_CHANGE     ((GPIO->P[gpioPortC].DIN >> 4) & 0x1)
...
#define RS485_CS_OFF         	(GPIO->P[gpioPortB].DOUTCLR = 1 << 8)
#define RS485_CS_ON          	(GPIO->P[gpioPortB].DOUTSET = 1 << 8)
...
#define ADS1230_POWERON     (GPIO->P[gpioPortB].DOUTSET = 1 << 13)//회로 수정후 적용
...
#define POWER_MODULE_ON     (GPIO->P[gpioPortC].DOUTSET = 1 << 5)
#define POWER_MODULE_OFF    (GPIO->P[gpioPortC].DOUTCLR = 1 << 5)
#define KEY_PORT_ZERO       ((GPIO->P[gpioPortD].DIN >> 7) & 0x1)
#define KEY_PORT_TARE       ((GPIO->P[gpioPortD].DIN >> 8) & 0x1)
#define KEY_PORT_MODE_HOLD  ((GPIO->P[gpioPortC].DIN >> 6) & 0x1)
...
#define RELAY_HIGH_ON        (GPIO->P[gpioPortB].DOUTSET = 1 << 6)           
#define RELAY_HIGH_OFF       (GPIO->P[gpioPortB].DOUTCLR = 1 << 6)
#define RELAY_OK_ON          (GPIO->P[gpioPortB].DOUTSET = 1 << 5)           
#define RELAY_OK_OFF         (GPIO->P[gpioPortB].DOUTCLR = 1 << 5)
#define RELAY_LOW_ON         (GPIO->P[gpioPortB].DOUTSET = 1 << 4)           
#define RELAY_LOW_OFF        (GPIO->P[gpioPortB].DOUTCLR = 1 << 4)
...
/*Enable clock for uart module*/  CMU_ClockEnable(cmuClock_USART1, true);\
...
/*KEY_UNIT_CHANGE key port init*/ GPIO_PinModeSet(gpioPortC, 4, gpioModeInputPull, 1);\
...
/*485_CS port int*/      		  GPIO_PinModeSet(gpioPortB, 8, gpioModeWiredAndPullUp, 0);\
/*UART1_TX port int*/             GPIO_PinModeSet(gpioPortD, 0, gpioModePushPull, 1);\
/*UART1_RX port int*/      		  GPIO_PinModeSet(gpioPortD, 1, gpioModeInput, 0);\
/*UART0_TX port int*/             GPIO_PinModeSet(gpioPortE, 10, gpioModePushPull, 1);\
/*UART0_RX port int*/      		  GPIO_PinModeSet(gpioPortE, 11, gpioModeInput, 0);\

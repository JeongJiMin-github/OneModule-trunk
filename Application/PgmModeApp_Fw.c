...
INT8U pgmFilterSetup(KEY_TYPE key)
{
	...
	switch (MenuFuncState)
	{
		...
		case 0x01:
			...
			return MENU_RET_EXIT;
#else
            tempInt32u = DispFiltParam.autozeroIndex;
            EditorSetDataAndRun(MenuDataWinNum, DEC, tempInt32u, 0, 999, 0, 0, 0);
            if (MenuWinIsOne) EditorSetConstStr("AT");
			else DispSetStr(WIN_WEIGHT, " AT");
            MenuFuncState = 0x10;       // autozero case
            break;
#endif
		...
		case 0x10:
            ...
            if ((tempInt32u / 100) < 2)
            {
                if(((tempInt32u % 100) / 10) <= 5)
                {
                    DispFiltParam.autozeroIndex = tempInt32u;
                    DispFiltWriteParam();
                }
            }
            return MENU_RET_EXIT;

...
void SaleModeSetDefaultParam(INT8U type)
{
	switch (type)
	{
		...
		case TURKEY:
			...
			/* 터키 PDN BLE 전용 CAL11 Setting */
			ParamWriteByte(PARAM_COMM_AREA_NUM, COMM_PARAM_OFFSET_BAUDRATE, BAUD_RATE_9600);
			ParamWriteByte(PARAM_COMM_AREA_NUM, COMM_PARAM_OFFSET_ECR_TYPE, ECR_TYPE_15);	// USB ECR TYPE#15(U-SET)
			ParamWriteByte(PARAM_COMM_AREA_NUM, COMM_PARAM_OFFSET_SELECT, RS232_BLE); 		// USE USB ECR / RS232 ECR
			break;
#endif

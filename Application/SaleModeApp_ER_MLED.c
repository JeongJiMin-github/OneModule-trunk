...
#define SALE_KEY_MAX_VOID_FUNC_NUM	27
#else
#define SALE_KEY_MAX_VOID_FUNC_NUM	28 //csh 20120730
#endif
/**
* @brief Key에 따른 Sale Function struct (void 함수)
* @remark keyNum == Key 이면 void func(void) 실행\n
...
INT8U saleDirectPluCall(KEY_TYPE key)
{
	...
	switch (SaleData.saleState)
	{
		...
		case SALE_STATE_PLU_TARE:			//TODO : 수정 위치
			SaleData.pluNum = tempPluNum;									
			if (saleUsePluNameFlag)
			{												
				salePluNamePosition = 0;
				tempPluTare = tempPluData.tare;
				//test -->
...
void salePluCallKey(void)
{
	...
	if (SaleData.unitPrice == 0 || SaleData.unitPrice > INDIRECT_PLU_MAX)
	{
		BuzzerSetOnTimes(2);
	}
	else
	...
	switch (SaleData.saleState)
	{
		case SALE_STATE_NORMAL:
			...
			if (AdData.grossZeroFlag)
			{
				...
				if (saleUsePluTareFlag)							// PLU tare를 사용하는 경우 
				{
					tempPluData.tare = 0;
					SaleData.saleState = SALE_STATE_PLU_TARE;
				}
				else											// PLU tare를 사용하지 않는경우 
				{
					SaleData.pluNum = 0;
...
INT8U saleFuncPluDataReadInSale(INT16U inputPluNum)
{
	...
	if (inputPluNum <= DIRECT_PLU_MAX)
	{
		...
		{				
			if (!tempPluData.tare && AdData.tareWeight && PluDirectData.tare)
			{
			...
			if((tempPluData.tare && !PluDirectData.tare) || (!AdData.tareWeight && PluDirectData.tare) || (tempPluData.tare && PluDirectData.tare)) i = AdSetPT(PluDirectData.tare, 1);			
			if (i != ERR_NONE)
			{ 
...
//		PrtPutConstStrWithCRLF(" (Kg)        \/Kg       \     ");			
		PrtPutTab(1);
		PrtPutConstStr("(");	PrtPutData(saleWeightUnit[SaleData.weightUnit], 2);		PrtPutConstStr(")");  
		PrtPutTab(3);	// Teb 2
...
{	
	...
	//		PrtPutConstStrWithCRLF(" xxxxx,xx xxxxxx,xx xxxxxxx,xx");
			PrtPutTab(1);		// Teb 2
			if (holdDisplayFlash != FALSE)
			{			
				PrtPutNum(SaleData.holdWeight, 7, SaleData.weightDecPos, weightDotTypeFlag, 0);
			}
			else
			{
			...
			{
				PrtPutLine();
				PrtPutConstStr("Sum(");	PrtPutStr(saleUsePriceUnit);	PrtPutConstStr(")");
				PrtPutTab(5);		// Teb 3
				if (saleUseEuroFlag) PrtPutNum(SaleData.totalPrice, 9, saleEuroDispDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
				else PrtPutNum(SaleData.totalPrice, 9, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
				PrtPutCRLF(9);

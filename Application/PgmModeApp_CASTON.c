...
//	INT8U 	retValue;
	...
	switch (MenuFuncState)
	{
		case 0x00:					// Auto Power OFF SET
			...
			{
				DispSetMsgWithNum("v", REAL_VERSION_NUM);
	...
	if (key == KEY_UNIT_CHANGE)
	{
		...
		else if(MenuFuncState == 0x03)		//Communication Set(CO,TU,ST)
		{
			if (tempInt8u != 3)
			{
				tempInt8u += 1;
		...
		else if(MenuFuncState == 0x04)		//ID Set
		{
			if (tempInt8u != 9)
			{
				tempInt8u += 1;
		...
		else if(MenuFuncState == 0x05)		//Bright Set
		{
			if (tempInt8u != 5)
			{
				tempInt8u += 1;
	...
	{		
		...
		zb16uEditData = (INT16U)tempInt32u;		
		EditorStop();
		ParamWriteWord(PARAM_SALE_AREA_NUM, SALE_PARAM_OFFSET_PCS_VALUE, zb16uEditData);		
//		ZigbeeInit(); 		//Zigbee 삭제

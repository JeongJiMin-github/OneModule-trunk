...
BOOLEAN SaleDecimalPointKeyOnOff; //csh 20120727
/** @brief bit 0, 입력할 숫자위치  */
INT8S SaleNumberPositonFlag; //csh 20120727
#endif
...
INT8U saleWeightUnit[5][5] = { "kg", "lb", "oz", "g ", "100g"};
/** @brief 가격 단위*/
#ifdef USE_HUNGARY_CURRENCY
INT8U salePriceUnit[2][3] = { "Ft", "\x80" }; //0-non euro. 1-Euro //see saleUseEuroFlag
#else
INT8U salePriceUnit[2][3] = { "$", "\x80" }; //0-non euro. 1-Euro //see saleUseEuroFlag
#endif
#else
/** @brief 무게 단위*/
//const INT8U code saleWeightUnit[4][3] = { "kg", "lb", "oz", "g "};
...
/** @brief Send Slovackia protocol flag*/
BOOLEAN saleSendSKProtocolflag;
#endif
...
#endif
/** @brief Date format related*/
static INT8U saleTareType;
static INT8U saleDateType;

#ifdef USE_MULTI_PRINT_FUNC			//#ifdef MultiPrint
/** @brief Multiple Print function related*/
static INT8U salePrintCopies = 0;			//required copies, user input	
static INT8U salePrevState;					//previous state store
static INT8U salePrintCounts = 0;			//print counts
static BOOLEAN saleMultiPrintEnable = 0;	//activate multiple print function
#endif			//#ifdef MultiPrint

#if defined (COUNTRY_CZECH) && defined (DIALOG6)
INT32S recentlySendWeight = 0;
#endif
...
#define ADD_KEY_TIMEOUT 200
#else
#define ADD_KEY_TIMEOUT 20
#endif
#endif
...
#ifdef	USE_DECIMAL_POINT
static void saleDecimalPointKey(void); //csh 20120727
#endif
#ifdef USE_ADD_KEY_FOR_CERTIFICATION
...
#ifdef USE_SLOVAKIA_PROTOCOL
static void saleSendSKprotocol(void);
#endif
...
#ifdef COUNTRY_CZECH
static void 	saleFuncPluDataWithoutTareWriteInSale(INT16U inputPluNum);
#endif
#ifdef DIALOG6
//wcm0816 code optimaization
//static void 	saleFuncPluDataChangeSale(INT16U inputPluNum);
//static void		SalePluDataCopyExt(INT8U *pluName, INT32U tempUnitPrice, INT32U tempTare);
...
static void salePrtArgoxStartCondition(INT8U printType);
static void salePrtArgoxPluName(void);
static void salePrtArgoxHeadMsg();
#else//DLP-50
static void salePrtDlpStartCondition(INT8U printType);

static void salePrtDlpPluName(void);
#endif//#ifdef USE_ARGOX_PRINTER
static void salePrtDate();
static void salePrtDlp(INT8U winNum);
#endif
#ifdef USE_MULTI_PRINT_FUNC			//#ifdef MultiPrint
static void saleMultiPrintFunc(void);  // multiple print function, (cycles in salemodeporc), kmhnsb 090720
#endif 			//#ifdef MultiPrint
#ifdef USE_CLERK_KEY
...
const SALE_KEY_FUNC code saleKeyFuncTable[SALE_KEY_MAX_FUNC_NUM] = 
{
	...
	{KEY_PLU_1, 	KEY_PLU_45, 	saleDirectPluCall},
};

//

#ifndef NO_USE_EURO_KEY
#define EUR_KEY_FUNC_NUM	1
#else
#define EUR_KEY_FUNC_NUM	0
#endif

//#ifndef NO_USE_PRT_ENABLE_KEY
//#define PRT_ENABLE_KEY_FUNC_NUM	1
//#else
//#define PRT_ENABLE_KEY_FUNC_NUM	0
//#endif

//#ifndef NO_USE_DISPLAY_TEST_KEY
//#define DISPLAY_TEST_KEY_FUNC_NUM	1
//#else
//#define DISPLAY_TEST_KEY_FUNC_NUM	0
//#endif
...
#ifdef USE_SLOVAKIA_PROTOCOL
/** @brief Quick Store Key에 따른 Sale Function(void type) 개수 */
#define SEND_SLOVAKIA_KEY_FUNC_NUM 1
#else
#define SEND_SLOVAKIA_KEY_FUNC_NUM 0
#endif


/** @brief Key에 따른 Sale Function(void type) 개수 */
#ifndef	USE_DECIMAL_POINT
#define SALE_KEY_MAX_VOID_FUNC_NUM (26+CLERK_KEY_FUNC_NUM+BARCODE_PRINT_KEY_FUNC_NUM+EUR_KEY_FUNC_NUM+PLU_QUICK_STORE_KEY_FUNC_NUM+SEND_SLOVAKIA_KEY_FUNC_NUM)//+PRT_ENABLE_KEY_FUNC_NUM+DISPLAY_TEST_KEY_FUNC_NUM+HOLD_KEY_FUNC_NUM)
#else
#define SALE_KEY_MAX_VOID_FUNC_NUM (27+CLERK_KEY_FUNC_NUM+BARCODE_PRINT_KEY_FUNC_NUM+EUR_KEY_FUNC_NUM+PLU_QUICK_STORE_KEY_FUNC_NUM+SEND_SLOVAKIA_KEY_FUNC_NUM)//+PRT_ENABLE_KEY_FUNC_NUM+DISPLAY_TEST_KEY_FUNC_NUM+HOLD_KEY_FUNC_NUM) //csh 20120730
#endif
/**
* @brief Key에 따른 Sale Function struct (void 함수)
* @remark keyNum == Key 이면 void func(void) 실행\n
...
#ifdef	USE_DECIMAL_POINT
{KEY_POINT,				saleDecimalPointKey}, //csh20120727
#endif
#ifdef USE_QUICK_STORE_FUNCTION
...
#endif
#ifdef USE_SLOVAKIA_PROTOCOL
	{KEY_SEND_SK_PROTOCOL,  saleSendSKprotocol},
#endif
};
/*
********************************************************************************
...
//	Use Standby Time Menu
	...
	#ifdef USE_MULTI_PRINT_FUNC			//#ifdef MultiPrint
	scaleUseMultiPrintFlag = (readSetDataInSale & 0x04)>>2; 	//read C45-2nd bit
	#endif			//#ifdef MultiPrint
...
SCALE_MODE_TYPE SaleModeProc(void)
{
	...
	INT8U funcNum; 			//byte = unreferenced value
	...
	switch (SaleData.addState)
	{
		...
		case 4:
			if (SaleData.printerState == 1)
			{
				...
				if(saleWaitPrinterResponseCount == ADD_KEY_TIMEOUT)
				{
					BuzzerSetOnTimes(2);
	...
	if(scaleUseMultiPrintFlag && scaleUsePrt == PRT_DLP50)	//Multiple Print function, on/off in C45 - 090721, kmhnsb
	{
		if(saleMultiPrintEnable)		//enables in multiprint state
			{
				saleMultiPrintFunc();
			}
	}
...
#if defined(USE_BT_COMM_API) || defined(USE_TM_COMM_API) || defined(USE_COMMAND_TARE_KEY) || defined(USE_SATO_PRINTER_PROTOCOL)	//kkh0225 code optimaization
/**
********************************************************************************
* @brief    Sale Mode를 돌면서 timer를 체크하는 함수
* @param    none
...
/**
********************************************************************************
* @brief    ER PLUS ECR에서 PLU data를  받아오는 함수
* @param    none
* @return   none
* @remark   
********************************************************************************
...
void receivedPluDataApply(INT8U *pluName, INT32U tempUnitPrice, INT32U tempTare)
{
...
#ifdef COUNTRY_CZECH
	if(pluName == 0) tempPluData.name[0] = 0x00;//PLU name을 전송하지 않을 경우 Total Price창에 공백이 아니라 Total Price(0.00)표기 요청
#endif
	if(SaleData.pluNum)
	{
...
#ifdef COUNTRY_CZECH
		if ((tempTare != 0xFFFFFFFF) && (AdData.tareWeight == 0))
			saleFuncPluDataWriteInSale(SaleData.pluNum);
		else
			saleFuncPluDataWithoutTareWriteInSale(SaleData.pluNum);
#else
		saleFuncPluDataWriteInSale(SaleData.pluNum);
#endif
	...
	if (tempTare != 0xFFFFFFFF) AdSetPT(tempTare,0);														// tare
...
//wcm0816 code optimaization
/*
void SalePluDataCopyExt(INT8U *pluName, INT32U tempUnitPrice, INT32U tempTare)
{
	INT8U i, noflag = 0;
	DIRECT_PLU_DATA tmpPLU;

	if (SaleData.pluNum > DIRECT_PLU_MAX)// || SaleData.pluNum < 1)
	{
			BuzzerSetOnTimes(2);
	}
	else
	{
		...
		if(SaleData.pluNum)
		{
			saleFuncPluDataChangeSale(SaleData.pluNum);
		}
		else
		{
			SaleData.pluNum = 1;
...
#ifdef USE_UNIT_SPENISH_OZ
		if(scaleUseKgUnitFlag ||scaleUseOzUnitFlag || scaleUseLbUnitFlag)
#else
		if(((scaleUseKgUnitFlag ||scaleUseOzUnitFlag) && scaleUseLbUnitFlag))
#endif
		{
...
#ifdef USE_CTS_TRIANGLE //미사용으로 결정
...
/////
...
#if defined (COUNTRY_CZECH) && defined (DIALOG6) 
					else if(tempPluData.name[0] == 0x00 && !saleUseEuroFlag) saleDispNumWithDecPoint(WIN_TOTAL_PRICE, priceDecPos, priceDotTypeFlag, SaleData.totalPrice, saleUseThousandPtFlag);
#endif
		...
		case SALE_STATE_MULTI_PRINT :
			DispSetStr(WIN_WEIGHT, "COPIES");
			saleDispNumWithDecPoint(WIN_UNIT_PRICE, 0, 0, salePrintCopies, 0);
			break;
#endif			//#ifdef MultiPrint
	...
	switch (SaleData.saleState)		/* 	input func	*/ 
	{
		...
		case SALE_STATE_NORMAL:
			...
			if (saleUnitPriceRenewalFlag == ON)
			{
				SaleData.unitPrice = 0;
#ifdef	USE_DECIMAL_POINT	
				SaleDecimalPointKeyOnOff = OFF; //csh 20120727 
#endif
			}
...
#ifdef USE_MULTI_PRINT_FUNC 		//#ifdef MultiPrint
		case SALE_STATE_MULTI_PRINT:
			salePrintCopies = EditorInsDecDigit(salePrintCopies, key - KEY_NUM_0, 2, 99, TRUE);
			break;
#endif		//#ifdef MultiPrint
...
INT8U saleDirectPluCall(KEY_TYPE key)
{
	...
	if (tempPluNum == 0 || tempPluNum > DIRECT_PLU_MAX)
	{
		BuzzerSetOnTimes(2);		
		return SCALE_MODE_SALE;	
	}
...
void salePluCallKey(void)
{
	...
	if (SaleData.unitPrice == 0 || SaleData.unitPrice > DIRECT_PLU_MAX)
	{
		BuzzerSetOnTimes(2);
	}
	else
...
void saleSumKey(void)
{
	...
	{	
		...
		case SALE_STATE_NORMAL:
			...
			else
			{
				if (AdData.stableFlag)
				{
					...
					else if (SaleData.totalPrice == 0 && SaleData.unitPrice > 0 && (!SaleData.addCount) && (!SaleData.weight))  // FIXED 판매	
					{
						saleEnableSumPrt = 1;
						salePrtLineFlag = 1;

#ifdef USE_MULTI_PRINT_FUNC		//#ifdef MultiPrint
						if(scaleUseMultiPrintFlag && (scaleUsePrt == PRT_DLP50))
						{
							salePrevState = SALE_STATE_MULTI_PRINT_FIXED;
					...
					else if (SaleData.totalPrice > 0 && (!SaleData.addCount))			// WEIGHT판매
					{				
						saleEnableSumPrt = 1;
						salePrtLineFlag = 1;
#ifdef USE_MULTI_PRINT_FUNC		//#ifdef MultiPrint
						if(scaleUseMultiPrintFlag && (scaleUsePrt == PRT_DLP50))
						{
							salePrevState = SALE_STATE_MULTI_PRINT_WEIGH;
							SaleData.saleState = SALE_STATE_MULTI_PRINT;
							break;
						}		
						else 
		...
		case SALE_STATE_MULTI:										//MULTI 판매
			...
			else
			{
				...
				if(scaleUseMultiPrintFlag && (scaleUsePrt == PRT_DLP50))
				{
					salePrevState = SALE_STATE_MULTI_PRINT_MULTI;
					SaleData.saleState = SALE_STATE_MULTI_PRINT;
					break;
				}	
				else 
...
#ifdef USE_MULTI_PRINT_FUNC		//#ifdef MultiPrint			
		case SALE_STATE_MULTI_PRINT:		//Multi print 모드시 SUM 키 입력으로 fixed, multi, weighed state(연속 인쇄 )로 변경
			if (AdData.stableFlag)
			{
				saleMultiPrintEnable = 1;
			}
			else
			{
				BuzzerSetOnTimes(2);
...
/**
********************************************************************************
* @brief    saleMultiPrintFunc 함수    
* @param    none
* @return   none
* @remark   multi print 모드에서 입력된 값 만큼 연속으로 print를 수행한다. (cycles in sale procedure)
********************************************************************************
*/
#ifdef USE_MULTI_PRINT_FUNC		//#ifdef MultiPrint			
void saleMultiPrintFunc(void)
{
	switch(salePrevState)
	{
		case SALE_STATE_MULTI_PRINT_FIXED:
			if(salePrintCounts<salePrintCopies)
			{
				salePrtFixedItem();
				salePrintCounts++;
				if (saleUseDailyTotalFlag) 
			...
			else
			{
				salePrintCopies = 0;
				transactionEndFlag = TRUE;
				saleFuncClearUnitPrice();				// FIXED판매가 이루어진 경우	
				saleHalfQuartEnable = 1;
				salePrintCounts = 0;
				saleMultiPrintEnable = 0;
				SaleData.saleState = SALE_STATE_NORMAL;
		...
		case SALE_STATE_MULTI_PRINT_WEIGH:	
			if(salePrintCounts<salePrintCopies)
			{
				salePrtWeightItem();
				salePrintCounts++;
			...
			else
			{
				salePrintCopies = 0;
				transactionEndFlag = TRUE;
				saleFuncClearUnitPrice();				// WEIGH 판매가 이루어진 경우 
				saleHalfQuartEnable = 1;
				salePrintCounts = 0;
				saleMultiPrintEnable = 0;
				SaleData.saleState = SALE_STATE_NORMAL;
		...
		case SALE_STATE_MULTI_PRINT_MULTI:			
			if(salePrintCounts<salePrintCopies)
			{
				salePrtMultiItem();
				salePrintCounts++;
			...
			else
			{
				salePrintCopies = 0;
				transactionEndFlag = TRUE;
				saleFuncClearUnitPrice();				// MULTI 판매가 이루어진 경우 
				saleHalfQuartEnable = 1;
				salePrintCounts = 0;
				saleMultiPrintEnable = 0;
				saleMultiCount = 0; //add kkanno 080326
...
********************************************************************************
*/
void saleDecimalPointKey(void) //csh 20120727
{
	...
	switch (SaleData.saleState)
	{
		...
		case SALE_STATE_MULTI:
			...
			{					
				...
				if (saleFirstAdd) salePrtLineFlag = 1;
				if (scaleUsePrt == PRT_DEP50) salePrtMultiItem();		//DLP 사용시에는 ADD키로 PRINT 불가, Debugged
				saleMultiCount = 0;			//add kkanno 080326
				if (saleFirstAdd) saleFirstAdd = 0;
				saleHalfQuartEnable = 1;
				SaleData.saleState = SALE_STATE_ADD;
...
void saleCancelKey(void)
{
	...
	{	
		...
		case SALE_STATE_MODE_SUM:
#ifdef DAILY_TOTAL_PRINT			
			if(dailyTotalReport.count <= 0)
			{	
				BuzzerSetOnTimes(2);
			}
			...
			else
			{
				if(scaleUsePrt == PRT_DEP50)
				{
					...
					PrtPutCRLF(1);
					PrtPutLine();
					PrtPutConstStr("Total Count : ");
					PrtPutNum(dailyTotalReport.count, 6, 0, weightDotTypeFlag, 0);
					PrtPutCRLF(1);
					PrtPutConstStr("Total Price : ");
					PrtPutNum(dailyTotalReport.price, 6, SaleData.priceDecPos, weightDotTypeFlag, 0);
		...
		case SALE_STATE_MULTI_PRINT:
			...
			salePrintCounts = 0;
			saleMultiPrintEnable = 0;
			if(salePrevState == SALE_STATE_MULTI_PRINT_FIXED) 	salePrtFixedItemCancel();
			if(salePrevState == SALE_STATE_MULTI_PRINT_WEIGH) salePrtWeightItemCancel();
			SaleData.saleState = SALE_STATE_NORMAL;
			BuzzerSetOnTimes(2);
			break;
#endif		//#ifdef MultiPrint	
...
*/
	if (scaleTareClearFlag == 0 || saleTareClearFlag)			// Scale doesn't use tare clear	
	{
		saleTareClearFlag = FALSE;
	}
	else								// Scale possible setting TareClear
...
static void saleSendSKprotocol(void)
{
	if(SaleData.unitPrice < 10000 && SaleData.unitPrice != 0)
	{
		if(!saleSendSKProtocolflag)
		{
			saleSendSKProtocolflag = 1;
		}
		else
			BuzzerSetOnTimes(2);
	}
...
INT8U saleFuncPluDataReadInSale(INT16U inputPluNum)
{
	...
	if (inputPluNum <= DIRECT_PLU_MAX)
	{
		...
		if (j > PLU_NAME_SIZE - 1)
		{
			...
			for(k=6;k<10;k++)	//kkh0225 code optimaization
			{
				saleDefaultPluName[k] = ' ';
			}
			//saleDefaultPluName[6] = ' ';
			//saleDefaultPluName[7] = ' ';
			//saleDefaultPluName[8] = ' ';	//add kkanno 071022
			//saleDefaultPluName[9] = ' ';	//add kkanno 071022
		}
#else
		if (j > PLU_NAME_SIZE - 1)
		{
			...
			//saleDefaultPluName[8] = ' ';	//add kkanno 071022
			//saleDefaultPluName[9] = ' ';	//add kkanno 071022
			//saleDefaultPluName[10] = ' ';	//add kkanno 071107
			//saleDefaultPluName[11] = ' ';	//add kkanno 071107
			//saleDefaultPluName[12] = ' ';	//add kkanno 071107
			//saleDefaultPluName[13] = ' ';	//add kkanno 071107
			//saleDefaultPluName[14] = ' ';	//add kkanno 071107
			//saleDefaultPluName[15] = ' ';	//add kkanno 071107
		}
#endif
		else
		{
...
*/
#ifdef COUNTRY_CZECH
void saleFuncPluDataWithoutTareWriteInSale(INT16U inputPluNum)
{
	if (inputPluNum > 0 && inputPluNum <= DIRECT_PLU_MAX)
	{
		PluDirectData = tempPluData;
...
*/
/*
void saleFuncPluDataChangeSale(INT16U inputPluNum)
{
	if (inputPluNum > 0 && inputPluNum <= DIRECT_PLU_MAX)
	{
		PluDirectData = tempPluData;
...
#ifdef DAILY_TOTAL_PRINT
		if(SaleData.saleState==SALE_STATE_MODE_SUM)//insert for daily total Print function.
		{		
				PrtPutLine();
				PrtPutCRLF(1);
				PrtPutTab(1);
				PrtPutConstStr("        Daily Total");
				PrtPutCRLF(1);
		}
		else{
#endif			
...
void salePrtStartCondition(void)
{
	...
	PrtPutChar(BEL);
	PrtPutChar(NULL); 
	//Aligining the characters for center
	PrtPutChar(ESC);
	PrtPutChar(0x61);//'='
	PrtPutChar(0x31); //'1' 
	//Font B
...
#endif
#if defined USE_ARGOX_PRINTER
/**
********************************************************************************
* @brief    데이타 출력시 weight item에 대한 판매 정보 출력  (ARGOX)   
* @param    none
* @return   none
* @remark   
********************************************************************************
...
void salePrtWeightItem(void)
{
	if (salePrtEnableFlag)
	{		
		if(scaleUsePrt == PRT_DLP50)//Argox Label
		{
			salePrtArgoxStartCondition(TYPE_NONE);
...
//===========================================Print out Barcode=====================================//			
			PrtPutDecNum(saleWeightPrefix, 2, PRT_NUM_TYPE_FILL_ZERO);								//V04 : Prefix(Barcode)			
			if(SaleData.pluNum)																		//V04 : itemCode(Barcode)
			{
				PrtPutDecNum(PluDirectData.itemCode, 5, PRT_NUM_TYPE_FILL_ZERO);					
			}
			else
			{
				PrtPutDecNum(0, 5, PRT_NUM_TYPE_FILL_ZERO);											
			}

			if (PrtSetting.barcodeType == TYPE_PRICE)
			{
				PrtPutDecNum(SaleData.totalPrice, 5, PRT_NUM_TYPE_FILL_ZERO); 						//V04 : Total Price(Barcode)
			}
			else if (PrtSetting.barcodeType == TYPE_WEIGHT) 
			{
				PrtPutDecNum(SaleData.weight, 6, PRT_NUM_TYPE_FILL_ZERO);							//V04 : Weight(Barcode)
			}
			PrtPutChar(LF);
//===========================================Print out Barcode=====================================//			

			salePrtArgoxPluName();																	//V07 : PLU name
																									//V08 : PLU number
...
#elif defined  USE_CITIZEN_PRINTER
/**
********************************************************************************
* @brief    데이타 출력시 weight item에 대한 판매 정보 출력 (DLP/DEP)    
* @param    none
* @return   none
* @remark   
********************************************************************************
...
void salePrtWeightItem(void)
{
	...
	if (saleUseEuroFlag) decPos = saleEuroDispDecPos;
	else decPos = priceDecPos;
	...
	{		
		if(scaleUsePrt == PRT_DLP50)
		{
			...
			//aligning the characters for center
			PrtPutChar(ESC);
			PrtPutChar(0x61);//'a'
			PrtPutChar(0x31);//'1'
			...
			/******ER PLUS MEP OIML 인증 : DEP PRINT시 프린트 자리수 맞추기 위해 변경. 9 -> 8 *********/		
//			if (saleUseEuroFlag) PrtPutNum(SaleData.unitPrice, 8, saleEuroDispDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
//			else PrtPutNum(SaleData.unitPrice, 8, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
			PrtPutNum(SaleData.unitPrice, 8, decPos, priceDotTypeFlag, saleUseThousandPtFlag);
			...
			//Tare
			PrtPutNum(AdData.tareWeight, 7, SaleData.weightDecPos, weightDotTypeFlag, 0);
...
//			else PrtPutNum(SaleData.totalPrice, 9, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
			PrtPutNum(SaleData.totalPrice, 9, decPos, priceDotTypeFlag, saleUseThousandPtFlag);
...
//				else PrtPutNum(SaleData.totalPrice, 9, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
		...
		else if (scaleUsePrt == PRT_DLP50)
		{
			...
			PrtPutChar(LF);
#ifdef USE_PRT_DLP50_V15_TARE
			PrtPutNum(AdData.tareWeight, 6, SaleData.weightDecPos, weightDotTypeFlag, saleUseThousandPtFlag); //V15 : Tare Weight
			PrtPutCRLF(1);
#endif
			/*
			ParamReadData(PARAM_HEAD_AREA_NUM, HEAD_PARAM_OFFSET_LINE1, tempStr, HEAD_LINE1_SIZE);
			tempStr[HEAD_LINE1_SIZE] = '\0';
			PrtPutStr(tempStr);	
			PrtPutCRLF(1);	  										//V15 : test

			ParamReadData(PARAM_HEAD_AREA_NUM, HEAD_PARAM_OFFSET_LINE2, tempStr, HEAD_LINE2_SIZE);
			tempStr[HEAD_LINE2_SIZE] = '\0';
			PrtPutStr(tempStr);
			PrtPutCRLF(1);											//V16 : test

			ParamReadData(PARAM_HEAD_AREA_NUM, HEAD_PARAM_OFFSET_LINE3, tempStr, HEAD_LINE3_SIZE);
			tempStr[HEAD_LINE3_SIZE] = '\0';
			PrtPutStr(tempStr);
			PrtPutCRLF(1);
			*///for CANADA DLP HEAD1,2,3, 																//V17 : test
			PrtPutData("P1,1", 4);													//End command		
			PrtPutChar(LF);
		}
	}
...
/**
********************************************************************************
* @brief    데이타 출력시 Fixed_value item에 대한 판매 정보 출력 (ARGOX)    
* @param    none
* @return   none
* @remark   
********************************************************************************
...
void salePrtFixedItem(void)
{
	if (salePrtEnableFlag)
	{
		...
		{
			salePrtArgoxStartCondition(TYPE_NONE);
			PrtPutDecNum(1, 6, PRT_NUM_TYPE_FILL_SPACE);						//V00 : Weigth(1 PCS, fixed sale)
			PrtPutChar(LF); 		
			PrtPutData("pcs", 3);
			//PrtPutData(saleWeightUnit[SaleData.weightUnit], 2); 				//V01 : Weight Unit
			PrtPutChar(LF); 		
...
//===========================================Print out Barcode=====================================//			
			PrtPutDecNum(saleCountPrefix, 2, PRT_NUM_TYPE_FILL_ZERO);			//V04 : Prefix(Barcode)
			if(SaleData.pluNum)													//V04 : itemCode(Barcode)
			{
				PrtPutDecNum(PluDirectData.itemCode, 5, PRT_NUM_TYPE_FILL_ZERO); 
			}
			...
			{
				PrtPutDecNum(1, 5, PRT_NUM_TYPE_FILL_ZERO); 					//V04 : Weigth(Barcode)
			}
			PrtPutChar(LF); 
//===========================================Print out Barcode=====================================//			
...
/**
********************************************************************************
* @brief	데이타 출력시 Fixed_value item에 대한 판매 정보 출력(DLP,DEP)	 
* @param	none
* @return	none
* @remark	
********************************************************************************
...
//			else PrtPutNum(SaleData.unitPrice, 9, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
			PrtPutNum(SaleData.unitPrice, 9, decPos, priceDotTypeFlag, saleUseThousandPtFlag);
...
//				else PrtPutNum(SaleData.unitPrice, 9, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
		...
		else if (scaleUsePrt == PRT_DLP50)
		{
			...
			PrtPutChar(LF);
#ifdef USE_PRT_DLP50_V15_TARE
			PrtPutData("      ", 6); //V15 : Tare Weight
			PrtPutCRLF(1);
#endif
			/*
			ParamReadData(PARAM_HEAD_AREA_NUM, HEAD_PARAM_OFFSET_LINE1, tempStr, HEAD_LINE1_SIZE);
			tempStr[HEAD_LINE1_SIZE] = '\0';
			PrtPutStr(tempStr);									  	//V15 test
			PrtPutCRLF(1);
...
/**
********************************************************************************
* @brief    데이타 출력시 Mul_ti item에 대한 판매 정보 출력(ARGOX)     
* @param    none
* @return   none
* @remark   
********************************************************************************
...
{	
	if (salePrtEnableFlag)
	{
		...
		{
			salePrtArgoxStartCondition(TYPE_NONE);
			PrtPutDecNum(saleMultiCount, 6, PRT_NUM_TYPE_FILL_SPACE);			//V00 : Weight(PCS)
			PrtPutChar(LF); 		
			//PrtPutData(saleWeightUnit[SaleData.weightUnit], 2); 				//V01 : Weight Unit
			PrtPutData("pcs",3);
			PrtPutChar(LF);
			...
			if(SaleData.pluNum) 
			{
				PrtPutDecNum(PluDirectData.itemCode, 5, PRT_NUM_TYPE_FILL_ZERO);  //V04 : itemCode(Barcode)
			}
			else
			{
				PrtPutDecNum(0, 5, PRT_NUM_TYPE_FILL_ZERO);						
			...
			if (PrtSetting.barcodeType == TYPE_PRICE) 							
			{
				//PrtPutDecNum(SaleData.totalPrice, 5, PRT_NUM_TYPE_FILL_ZERO);	//V04 : UnitPrice(Barcode)
				PrtPutDecNum(SaleData.totalPrice, 5, PRT_NUM_TYPE_FILL_ZERO);	//V04 : Total Price(Barcode)
			}
			else if (PrtSetting.barcodeType == TYPE_WEIGHT)						//V04 :Weight(Barcode)
			{
				PrtPutDecNum(saleMultiCount, 5, PRT_NUM_TYPE_FILL_ZERO);	
			}
			PrtPutChar(LF);
...
//				else PrtPutNum(SaleData.totalPrice, 9, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
		...
		else if (scaleUsePrt == PRT_DLP50)
		{
			...
			PrtPutChar(LF);
#ifdef USE_PRT_DLP50_V15_TARE
			//S2000JR do not use multi-item sale
#endif
			/*
			ParamReadData(PARAM_HEAD_AREA_NUM, HEAD_PARAM_OFFSET_LINE1, tempStr, HEAD_LINE1_SIZE);
			tempStr[HEAD_LINE1_SIZE] = '\0';
...
//		PrtPutConstStrWithCRLF(" (CANCEL)  PLU-xx :           ");
	if (salePrtEnableFlag)
	{
		...
		PrtPutCRLF(1);	//아래 if문에 있는 부분 삭제하고 이곳에 추가
		if (saleUsePluNameFlag)
		{
//			PrtPutCRLF(1);	
			salePrtPluName();
...
//	else PrtPutNum(saleReceiveMoney, 9, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
	PrtPutNum(saleReceiveMoney, 9, decPos, priceDotTypeFlag, saleUseThousandPtFlag);
...
//	else PrtPutNum(saleChangeMoney, 9, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
	PrtPutNum(saleChangeMoney, 9, decPos, priceDotTypeFlag, saleUseThousandPtFlag);
...
********************************************************************************
*/
void salePrtArgoxStartCondition(INT8U printType)
{
	PrtPutConstStr("N\x0A");
	PrtPutConstStr("FK\x22LABEL_FORM\x22\x0A");
	PrtPutConstStr("FS\x22LABEL_FORM\x22\x0A");

	PrtPutConstStr("V00,10,N,\x22 \x22 \x0A");
	PrtPutConstStr("V01,10,N,\x22 \x22 \x0A");
	PrtPutConstStr("V02,10,N,\x22 \x22 \x0A");
	PrtPutConstStr("V03,10,N,\x22 \x22 \x0A");
	PrtPutConstStr("V04,12,N,\x22 \x22 \x0A");
	PrtPutConstStr("V07,28,C,\x22 \x22 \x0A");
	//PrtPutConstStr("V08,10,N,\x22 \x22 \x0A");
	PrtPutConstStr("V09,10,N,\x22 \x22 \x0A");
	//PrtPutConstStr("V10,10,N,\x22 \x22 \x0A");
	if(SaleData.pluNum && (tempPluData.validDate > 0))
	{
		PrtPutConstStr("V11,10,N,\x22 \x22 \x0A");
		PrtPutConstStr("V12,10,N,\x22 \x22 \x0A");
	}
	PrtPutConstStr("V13,29,C,\x22 \x22 \x0A");
	PrtPutConstStr("V14,29,C,\x22 \x22 \x0A");
	PrtPutConstStr("V15,33,C,\x22 \x22 \x0A");


	PrtPutConstStr("S4\x0A");						//Print Speed
	PrtPutConstStr("ZB\x0A");						//Print Direction
	PrtPutConstStr("q481\x0A"); 					//Width of Label
	PrtPutConstStr("Q340,30\x0A");					//Length and gab size of Label 331 10
	PrtPutConstStr("JF\x0A");						// Enable Back Feed /JB: disable Back Feed
	//JBD7S2OD
	
	PrtPutConstStr("A360,134,0,2,1,1,N,V00\x0A");	//V00 : Weight 380  
	PrtPutConstStr("A430,134,0,2,1,1,N,V01\x0A");	//V01 : Weight Unit 450
	PrtPutConstStr("A216,205,0,2,1,2,N,V02\x0A");	//V02 : Unit Price		
	PrtPutConstStr("A357,192,0,4,1,2,N,V03\x0A");	//V03 : Total Price
	PrtPutConstStr("B12,166,0,E30,2,6,81,B,V04\x0A"); 	//V04 : Barcode(Prefix 2, Itemcode 5, Weight 5 or TotalPrice 5)		
	PrtPutConstStr("A18,49,0,4,1,2,N,V07\x0A");		//V07 : PLU name
	//PrtPutConstStr("A22,134,0,1,1,1,N,V08\x0A");	//V08 : PLU number
	PrtPutConstStr("A300,207,0,1,1,2,N,V09\x0A");	//V09 : UnitPrice Unit
	//PrtPutConstStr("A443,204,0,1,1,3,N,V10\x0A");	//V10 : total price unit
	if(SaleData.pluNum && (tempPluData.validDate > 0))
	{
		PrtPutConstStr("A227,134,0,2,1,1,N,V11\x0A");	//V11 : Packed on date
		PrtPutConstStr("A321,154,0,2,1,1,N,V12\x0A");	//V12 : Sell by date
	}
	PrtPutConstStr("A13,262,0,4,1,1,N,V13\x0A");	//V10 : Head 1
	PrtPutConstStr("A13,287,0,4,1,1,N,V14\x0A");	//V11 : Head 2
	PrtPutConstStr("A17,310,0,3,1,1,N,V15\x0A");	//V12 : Head 3
	
	//PrtPutConstStr("FE\x0A");
	PrtPutData("FE",3);
	PrtPutChar(LF);
	
	PrtPutConstStr("FR\x22LABEL_FORM\x22\x0A");
	PrtPutChar(0x3F);	//?
	PrtPutChar(LF);
}
#else//DLP-50
...
/**
********************************************************************************
* @brief    ARGOX 에서 PLU name을 보내는 Protocol 함수
* @param  none
* @return   none
* @remark   
********************************************************************************
*/
void salePrtArgoxPluName(void)
{
	INT8U tempName[PLU_NAME_SIZE + 1];
	
	if(SaleData.pluNum)
	{
		if (SaleData.pluNum <= DIRECT_PLU_MAX)
		{
			if(saleDefaultPluName[0] != 0x00){
				MakeStrCopy(tempName, saleDefaultPluName, PLU_NAME_SIZE);
			}
			else{
				MakeStrCopy(tempName, tempPluData.name , PLU_NAME_SIZE);
			}
		}
		else{
			MakeStrCopy(tempName, "NO_PLU", PLU_NAME_SIZE);
	}
		DelStrSpace(tempName, PLU_NAME_SIZE);
		tempName[PLU_NAME_SIZE] = '\0';
		PrtPutStr(tempName);
		PrtPutChar(LF);
	}
	else
	{		
		MakeStrCopy(tempName, "NO_PLU", PLU_NAME_SIZE);
		DelStrSpace(tempName, PLU_NAME_SIZE + 1) ;
		tempName[PLU_NAME_SIZE] = '\0';
		PrtPutStr(tempName);
		PrtPutChar(LF);
	}
...
//			else PrtPutNum(SaleData.unitPrice, 7, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
			PrtPutNum(SaleData.unitPrice, 7, decPos, priceDotTypeFlag, saleUseThousandPtFlag);
			break;
...
//			else PrtPutNum(SaleData.totalPrice, 7, priceDecPos, priceDotTypeFlag, saleUseThousandPtFlag);
			PrtPutNum(SaleData.totalPrice, 7, decPos, priceDotTypeFlag, saleUseThousandPtFlag);
			break;
...
*/
#ifdef USE_ARGOX_PRINTER
void salePrtArgoxHeadMsg()
{
	INT8U	tempStr[HEAD_LINE1_SIZE + 1];
	
	ParamReadData(PARAM_HEAD_AREA_NUM, HEAD_PARAM_OFFSET_LINE1, tempStr, HEAD_LINE1_SIZE);
	tempStr[HEAD_LINE1_SIZE] = '\0';
	DelStrSpace(tempStr, HEAD_LINE1_SIZE);
	PrtPutStr(tempStr); 
	PrtPutCRLF(1);											//V15 : test
	
	ParamReadData(PARAM_HEAD_AREA_NUM, HEAD_PARAM_OFFSET_LINE2, tempStr, HEAD_LINE2_SIZE);
	tempStr[HEAD_LINE1_SIZE] = '\0';
	DelStrSpace(tempStr, HEAD_LINE2_SIZE);
	PrtPutStr(tempStr);
	PrtPutCRLF(1);											//V16 : test
	
	ParamReadData(PARAM_HEAD_AREA_NUM, HEAD_PARAM_OFFSET_LINE3, tempStr, HEAD_LINE3_SIZE);
	tempStr[HEAD_LINE1_SIZE] = '\0';
	DelStrSpace(tempStr, HEAD_LINE3_SIZE);
	PrtPutStr(tempStr);
	PrtPutCRLF(1);
}
#endif//#ifdef USE_ARGOX_PRINTER
...
*********************************************************************************
*/
#ifdef USE_DATE_SEPARATOR_SLASH
...
//				TimeGetNextDay(&year, &month, &date, (tempPluData.validDate - 1));
		...
		case 2:	//DD.MM.YY
			...
			//Add location V11 or more...													//V12 : Sell by date(MM.DD.YY)
			if(SaleData.pluNum && (tempPluData.validDate > 0))
			{
//				year = (INT16U)TimeData.year + 2000;
//				month = (INT16U)TimeData.month;
...
{
	return (INT32S)saleWeightPrefix;
}
#endif
#endif

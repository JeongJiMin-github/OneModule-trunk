...
BOOLEAN 	saleSend_CHNdataFlag;
INT8U 		saleSend_CHNdatatype;
#endif
...
#ifdef	USE_LONG_KEY_FOR_CHINAMARKET
static void saleSendCHNprotocol(void);
static void saleSetCHNprotocol(void);
#endif
...
#ifdef DIALOG6
static void receivedPluDataApply(INT8U *pluName, INT32U tempUnitPrice, INT32U tempTare);
#endif
...
#ifdef USE_LONG_KEY_FOR_CHINAMARKET
#define PROTOCOL_FOR_CHNMARKET			2
#else
#define PROTOCOL_FOR_CHNMARKET			0
#endif


/* Dealer Function */
#ifdef USE_PRINTER_FOR_CHINA
#define PRINTER_SIMPLE_COMPUTING		1
#else
#define PRINTER_SIMPLE_COMPUTING		0
#endif


#define SALE_KEY_MAX_VOID_FUNC_NUM	(28+USE_DECIMAL_POINT_FUNC_NUM+PLU_QUICK_STORE_KEY_FUNC_NUM+PROTOCOL_FOR_CHNMARKET+PRINTER_SIMPLE_COMPUTING)
...
#ifdef USE_PRINTER_FOR_CHINA
	{KEY_PRINT, 			salePrtWeightItem},
#endif
#ifdef USE_LONG_KEY_FOR_CHINAMARKET
	{KEY_SET_PROTOCOL, 		saleSetCHNprotocol},
	{KEY_SEND_PROTOCOL,		saleSendCHNprotocol},
#endif
...
//	Use Standby Time Menu
	...
	saleEnableSumPrt =0;
	salePrtLineFlag =1;
	saleUseHeadMsgFlag = 0;
	salePrtDayFlag = 0;
	saleMultiCount = 0;
	saleHundredGramEnable =0; 
	saleHundredGramPrice=0;	
#endif
#ifdef USE_LONG_KEY_FOR_CHINAMARKET
	tempInt8u = ParamReadByte(PARAM_DISP_AREA_NUM, DISP_PARAM_OFFSET_BLINK_COUNT);			//DISP_PARAM_OFFSET_BLINK_COUNT는 LED 제품에서 사용하지 않음
	saleSend_CHNdataFlag = tempInt8u & 0x10;
	saleSend_CHNdatatype = (tempInt8u & 0x0C)>>2;
#endif
...
void saleDisplayFunc(void)
{
	...
	switch (SaleData.saleState)		/*	display	*/
	{
		...
		case SALE_STATE_CHECK_BATTERY:  
			...
			break;
#ifdef USE_LONG_KEY_FOR_CHINAMARKET
		case SALE_STATE_SEND_CHNDATA:
			DispSetStr(WIN_WEIGHT, "TRANS");
			if(saleSend_CHNdataFlag)DispSetStr(WIN_UNIT_PRICE, "START");
			else DispSetStr(WIN_UNIT_PRICE, " STOP");
			DispSetStr(WIN_TOTAL_PRICE, "	   ");
			holdDisplayFlash++;
			if (holdDisplayFlash == 1000)
			{			
				holdDisplayFlash = 0;
				SaleData.saleState = SALE_STATE_NORMAL;
			}
		...
		case SALE_STATE_SET_CHNDATA:  
			DispSetStr(WIN_WEIGHT, "TRANS");
			DispSetStr(WIN_UNIT_PRICE, " TYPE");
			saleDispNumWithDecPoint(WIN_TOTAL_PRICE, 0, 0, saleSend_CHNdatatype+1, 0);			
			break;
#endif
		default:
...
/**
********************************************************************************
* @brief    Chine market protocol Send flag  변경을 위한 함수(toggle) 
* @param    none
* @return   none
* @remark   
********************************************************************************
*/
void saleSendCHNprotocol(void)
{
	INT8U tempByte;
	...
	switch (SaleData.saleState)
	{
		case SALE_STATE_NORMAL :
		case SALE_STATE_SEND_CHNDATA :
			SaleData.saleState = SALE_STATE_SEND_CHNDATA;

			tempByte = ParamReadByte(PARAM_DISP_AREA_NUM, DISP_PARAM_OFFSET_BLINK_COUNT);			//DISP_PARAM_OFFSET_BLINK_COUNT는 LED 제품에서 사용하지 않음

			if (tempByte & 0x10) tempByte = tempByte & ~(0x10);
			else tempByte = tempByte | (0x10);

			ParamWriteByte(PARAM_DISP_AREA_NUM, DISP_PARAM_OFFSET_BLINK_COUNT, tempByte);		//DISP_PARAM_OFFSET_BLINK_COUNT는 LED 제품에서 사용하지 않음

			saleSend_CHNdataFlag = tempByte & 0x10;
			break;
		default:
			BuzzerSetOnTimes(2);
			break;
...
/**
********************************************************************************
* @brief    Chine market protocol Send type 결정을 위한 함수
* @param    none
* @return   none
* @remark   
********************************************************************************
*/
void saleSetCHNprotocol(void)
{
	INT8U tempByte;
	...
	{
		case SALE_STATE_NORMAL :
			SaleData.saleState = SALE_STATE_SET_CHNDATA;
			tempByte = ParamReadByte(PARAM_DISP_AREA_NUM, DISP_PARAM_OFFSET_BLINK_COUNT);			//DISP_PARAM_OFFSET_BLINK_COUNT는 LED 제품에서 사용하지 않음
			saleSend_CHNdatatype = (tempByte>>2) & 0x03;
			break;
